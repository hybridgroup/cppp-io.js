...
var HTTP = module.exports = function HTTP(opts) {
  ...
  if (!this.host) {
    throw new Error("No host supplied for HTTP protocol");
  }

  if (!this.port) {
    throw new Error("No port supplied for HTTP protocol");
  }
...
HTTP.prototype.command = function command(name, endpoint, params, callback) {
  ...
  function handle(err, res) {
    if (err) { return _.resolve(callback, res.body && res.body.error || err); }
    if (!res.ok) { return _.resolve(callback, res.text); }
